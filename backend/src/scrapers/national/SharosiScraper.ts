...
export class SharosiScraper extends BaseScraper {
  constructor(officialUrl: string) {
    super('社会保険労務士 (社労士)', officialUrl);
  }

  async fetch(): Promise<ScraperResult> {
    try {
      const $ = await this.fetchHtml('https://www.sharosi-siken.or.jp/exam/');
      const result = this.buildResult();
      result.source_url = 'https://www.sharosi-siken.or.jp/exam/';
      result.exam_fee = '15,000円';
      ...
      result.note = result.exam_date
        ? `次回試験日: ${result.exam_date}（社会保険労務士試験オフィシャルサイトより取得）`
        : '社労士試験は年1回（8月第4日曜）実施。最新日程は公式サイトをご確認ください。';
